	...
	private Connection createNewConnection() {		
		...
		for(int counter = 0; counter < 10; counter++) {
			...
			try {
				...
				//System.out.println("Erro durante a conexão (Servidor MySQL está off-line). Tentando uma nova conexão com o servidor... (Tentativa " + (counter + 1) +")");				
			}
			catch (CommunicationsException e) {
				//System.out.println("Erro durante a conexão (Resposta do servidor MySQL inválida). Tentando uma nova conexão com o servidor... (Tentativa " + (counter + 1) +")");
			}		
		...
		for (i = 0; i < mySqlConnections.length; i++) {
			...
			try {
				if (connection.isClosed()) {
					//System.out.println("Conexão ao MySQL esta fechada. Reconectando...");
