...
public class PoolAimer extends YahooControl implements TimerHandler {
	...
	public static PoolBall cloneBall(PoolBall ball) {
		...
		poolBall.setCoords(ball.x, ball.y);
		...
		poolBall.vel.setCoords(ball.vel);
		poolBall.wX.setCoords(ball.wX);
		poolBall.h.setCoords(ball.h);
		poolBall.h.setVel(ball.h.getAbsVel());
		poolBall.sliding = ball.sliding;
		poolBall.willCollideWithAObstacle = ball.willCollideWithAObstacle;
		poolBall.L = ball.L;
	...
	public YPoint getFirstColl() {
		if (ballColided) {
			firstColl1.setCoords(firstColl.x, firstColl.y);
			return firstColl1;
		}
