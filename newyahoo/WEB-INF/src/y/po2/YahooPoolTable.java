...
import common.po2.EightBallSetup;
import common.po2.IBall;
import common.po2.NineBallSetup;
import common.po2.Obstacle;
import common.po2.Pool;
import common.po2.PoolBall;
import common.po2.PoolData;
import common.po2.PoolHandler;
...
public class YahooPoolTable extends YahooGamesTable implements PoolHandler,
	...
	public YPoint	lastCueDist;
	public YPoint	lastEnglishDist;
	public YPoint	lastFirstColl;
	public byte		lastCollBall;
	public Pool		pool;
	PoolArea		poolArea;
	PoolAimer		poolAimer;
	...
	public boolean	bestSrikeStarted	= false;
	...
	public YahooPoolTable() {
		...
		u = 0;
		v = 0;
		lastCueDist = new YPoint();
		lastEnglishDist = new YPoint();
		lastFirstColl = new YPoint();
		...
		update = 1;
		cueDist = new YPoint();
		englishDist = new YPoint();
		firstColl = new YPoint();
		ypt_I = new YVector();
		J = new PoolData();
		state = "";
	...
	public void actionChangePoolBall(int k1, float j2, float l2) {
	...
	public YPoint getCenterPoint() {
		return (YPoint) pool.getProperty("CENTER_POINT");
	}
	...
	public float getLinearFriction() {
		return pool.getFloatProperty("linearFriction");
	}
	...
	public float getRotationFriction() {
		return pool.getFloatProperty("rotationFriction");
	}

	public float getSideRotationFriction() {
		return pool.getFloatProperty("sideRotationFriction");
	}
	...
	public void handleSetPos(int i1, int j1, float k1, float l1,
			YVector _pcls48, Vel _pcls33) {
		if (_pcls48 != null)
			ypt_I.setCoords(_pcls48.x, _pcls48.y);
		else
	...
	public void handleWon(YData _pcls111) {
		String s1 = "YOU";
		poolArea.hideCue();
		if (!pool.training)
	...
	public boolean haveInitTimePorMove() {
		Integer tm = (Integer) pool.getProperty("INIT_TIME_PER_MOVE");
		return tm != null && tm > 0;
	}
	...
	public void Jd() {
		...
		if (pool.isRunning()) {
			...
			if (i1 != -1)
				poolArea.cueSprite.Tk((YPoint) pool.getBall(i1));
		}
		Hd();
		beep();
	}
	...
	public synchronized void parseData(byte byte0,
		...
		case -102: // 9A: change pool ball
			...
			float j2 = datainputstream.readFloat();
			float l2 = datainputstream.readFloat();
		...
		case -126: // 82: strike
			...
			lastCueDist.setCoords(cueDist.x, cueDist.y);
			englishDist.read(datainputstream);
			lastEnglishDist.setCoords(englishDist.x, englishDist.y);
			firstColl.read(datainputstream);
			lastFirstColl.setCoords(firstColl.x, firstColl.y);
	...
	public void strike() {
		if (isMyTurn() && pool.isRunning() && pool.getCurrentState() == 0) {
			...
			YPoint cueDist = new YPoint(poolArea.cueSprite.getPos(true));
			YPoint englishDist = new YPoint(poolArea.english.getPos());
			YPoint firstColl = new YPoint(poolAimer.getFirstColl());
			int collBall = poolAimer.getIndex();
			logMessage("***moving on", Color.green);
			boolean flag = pool.Bj(selectedBall);
			if (flag) {
				YVector u = new YVector((YPoint) selectedBall, cueDist);
				float f1 = 5F;
				if (collBall == 1)
					f1 *= 1.3F;
				float l1;
				l1 = (float) ((Math.random() - 0.5F) * f1);
				u.mul(1.4F);
				u.y += l1;
				cueDist = new YPoint((PoolBall) selectedBall);
				cueDist.add(u);
				firstColl = new YPoint(0, 0);
			}
			if (collBall != -1) {
				IBall _ball[] = pool.getBall();
				for (IBall item : _ball) {
					if (item.getIndex() != collBall
							&& item.distance(firstColl) < 21)
						collBall = -1;
				}
	...
	public void strike(int index, YPoint cueDist, YPoint englishDist,
			YPoint firstColl, int collBall) {
	...
	public void updatePB(int i1, float j1, float k1) {
		if (isMyTurn() && pool.isRunning() && pool.getCurrentState() == 0) {
