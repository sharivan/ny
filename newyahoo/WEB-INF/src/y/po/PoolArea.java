...
public class PoolArea extends YahooControl {
	...
	public boolean cc(Event event, int i1, int j1) {
		...
		for (int l1 = 0; l1 < slot.length; l1++)
			if (slot[l1].Py(PoolMath.intToYInt(i1), PoolMath.intToYInt(j1))) {
				handler.setSlot(l1);
				return true;
			}
		if (poolAimer != null)
	...
	public boolean dc(Event event, int i1, int j1) {
		if (pa_D) {
			if (handler.getPool().Fj(handler.getMySitIndex(), pa_E.getIndex(),
					PoolMath.intToYInt(i1), PoolMath.intToYInt(j1))) {
				handler.updatePB(pa_E.getIndex(), PoolMath.intToYInt(i1),
						PoolMath.intToYInt(j1));
			}
			else {
				pa_E.Wd();
				activate();
	...
	public void fc(int index, int cmd, int x, int y, YVector wX, Vel h) {
	...
	public void gc() {
		...
		if (_lcls164 != null && poolAimer != null)
			poolAimer.os((int) PoolMath.yintToFloat(((YIPoint) _lcls164).a),
					(int) PoolMath.yintToFloat(((YIPoint) _lcls164).b));
	...
	public boolean kc(Event event, int i1, int j1) {
		for (int k1 = 0; k1 < ballSprite.length; k1++) {
			if (ballSprite[k1].isMoving()
					...
					|| ballSprite[k1].getInSlot())
				continue;
			cueSprite.Tk(new YPoint(ballSprite[k1].poolBall.getYIntX(),
					ballSprite[k1].poolBall.getYIntY()));
			break;
		}
