...
public class CueSprite extends YahooComponent implements YData {
	...
	public boolean Kk(Event event, int i1, int j1) {
		...
		if (isActive() && isPulled()) {
			if (m_selectedBall.distance(m_ball.copy()) > m_selectedBall
					.getRadius())
				strike();
			else
				setPulled(false);
	...
	public void read(DataInput input) throws IOException {
		input.readInt();
		float f1 = PoolMath.yintToFloat(input.readInt());
		float f2 = PoolMath.yintToFloat(input.readInt());
		float f3 = PoolMath.yintToFloat(input.readInt());
		float f4 = PoolMath.yintToFloat(input.readInt());
	...
	public void Tk(YPoint _pcls169) {
		if (!isActive() || m_selectedBall == null)
			return;
		if (_pcls169.distance(m_selectedBall.getYIntX(), m_selectedBall
				.getYIntY()) < 11F) {
			return;
		}
		V.setCoords(m_selectedBall.getYIntX() - _pcls169.x, m_selectedBall
				.getYIntY()
				- _pcls169.y);
		_pcls169
				.setCoords(m_selectedBall.getYIntX(), m_selectedBall.getYIntY());
		_pcls169.add(V);
		Vk(_pcls169);
		return;
	}
	...
	public void Vk(YPoint _pcls169) {
		if (m_selectedBall != null) {
			m_cue.setCoords(_pcls169.x - m_selectedBall.getYIntX(), _pcls169.y
					- m_selectedBall.getYIntY());
			if (m_cue.x == 0.0F && m_cue.y == 0.0F)
				return;
			m_cue.setCoordsTo(V);
			V.versor();
			V.mul(N);
			m_ball.setCoords(m_selectedBall.getYIntX() + V.x, m_selectedBall
					.getYIntY()
					+ V.y);
			m_cue.versor();
			m_cue.mul(O);
	...
	public void Wk() {
		if (m_selectedBall != null && !m_selectedBall.inSlot()) {
			m_cue.setCoords(m_selectedBall.getYIntX() - cs_s.x, m_selectedBall
					.getYIntY()
					- cs_s.y);
			m_cue.setCoordsTo(V);
			V.versor();
	...
	public void write(DataOutput output) throws IOException {
		...
		output.writeInt(PoolMath.floatToYInt(m_ball.x));
		output.writeInt(PoolMath.floatToYInt(m_ball.y));
