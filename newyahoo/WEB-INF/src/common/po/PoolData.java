...
public class PoolData implements YData {
	...
	public String clientTs() {
		...
		for (int i = 0; i < turnInArea.getCount();) {
			...
			s = s + "[" + j;
			int k = turnInArea.getInteger(i++);
			s = s + " " + PoolMath.yintToFloat(k);
			int l = turnInArea.getInteger(i++);
			s = s + " " + PoolMath.yintToFloat(l);
	...
	public void read(YIPoint[] points) {
		turnCollided = true;
		c = false;
		firstCollidedBall = -1;
		...
		for (YIPoint point : points) {
			turnInArea.writeInt(point.a);
			turnInArea.writeInt(point.b);
		}
		cleared = false;
	}
