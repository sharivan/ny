...
public class Pool extends Game implements PoolConsts, PoolEngineHandler,
	...
	private YPoint						cueDist;

	private YPoint						englishDist;

	private YPoint						firstColl;
	...
	public boolean doNotifyTELAPS(PoolData _pcls131) {
		if (_pcls131 != null) {
			...
			for (int i1 = 0; i1 < _pcls131.turnInArea.getCount(); setPos(index,
				...
				x = _pcls131.turnInArea.getFloat(i1++);
				y = _pcls131.turnInArea.getFloat(i1++);
	...
	public boolean doNotifyTStat(PoolData _pcls131, boolean flag) {
		...
		for (int j1 = 0; j1 < _pcls131.turnInArea.getCount(); setPos(index, x,
			...
			x = _pcls131.turnInArea.getFloat(j1++);
			y = _pcls131.turnInArea.getFloat(j1++);
	...
	public boolean doStrike(int turn, int _index, YPoint _cueDist,
			YPoint _englishDist, YPoint _firstColl, int _collBall) {
		if (m_turn != turn)
			return false;
		if (m_currentState != 0)
			return false;
	...
	public boolean doUpdatePB(int turn, int index, float x, float y) {
	...
	public boolean Fj(int turn, int index, float x, float y) {
	...
	public float getFloatProperty(String s1) {
	...
	public void handleCollBalls(IBall _pcls124, IBall _pcls124_1) {
		...
		if (handler != null)
			if (_pcls124.getVel().abs() > 5 || _pcls124_1.getVel().abs() > 5)
	...
	public boolean isFirstStrike() {
		...
		for (IBall _lcls124 : ball) {
			if (_lcls124.equals(setup.getWhiteBall())
					|| setup.getInitPos(_lcls124.getIndex()).equals(_lcls124))
				continue;
			flag = false;
			break;
		}
	...
	public void setup() {
		...
		for (int j1 = 0; j1 < i1; j1++) {
			...
			YPoint p = poolSetup.getInitPos(j1);
			PoolBall _lcls171 = new PoolBall();
			_lcls171.index = j1;
			_lcls171.setCoords(p.x, p.y);
			_lcls171.x0 = p.x;
			_lcls171.y0 = p.y;
			_lcls171.radius = 10;
			_lcls171.m_inSlot = -1;
	...
	public void Sj(IBall _pcls124, float i1, float j1) {
	...
	public PoolData tj() {
		...
		for (IBall element : ball) {
			...
			G.turnInArea.writeFloat(element.getX());
			G.turnInArea.writeFloat(element.getY());
	...
	public boolean useInitTimePorMove() {
		return getFloatProperty("INIT_TIME_PER_MOVE") != -1;
	}
	...
	public boolean yj() {
		...
		for (IBall _lcls124_1 : ball) {
			if (_lcls124_1.equals(_lcls124)
					|| getSetup().getInitPos(_lcls124_1.getIndex()).equals(
							(YPoint) _lcls124_1))
				continue;
			flag = false;
			break;
